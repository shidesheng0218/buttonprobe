...
program
  ...
  .option("--patch <file>", "unified diff generated by an external agent")
  .option("--patch-url <url>", "remote unified diff URL, for example a GitHub pull request .diff URL")
  ...
  .option("--timeout <milliseconds>", "post-click observation window", integer)
  .option("--target <selector-or-id>", "target control selector or ButtonProbe control id")
  .option("--unsafe", "allow controls with destructive labels")
  ...
  .action(async (url: string, _flags, command) => {
    ...
    const result = await runPatchVerification({
      baseUrl: url,
      ...(flags.patch ? { patchPath: resolve(flags.patch) } : {}),
      ...(flags.patchUrl ? { patchUrl: flags.patchUrl } : {}),
      projectRoot,
      ...
      ...(flags.target ? { targetSelector: flags.target } : {}),
