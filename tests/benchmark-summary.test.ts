...
test("creates a README benchmark block from actual eval JSON", async () => {
  ...
  await updateBenchmarkSummary({ viralPath: viral, reactPath: react, outputPath: output, readmePath: readme });

  const latest = JSON.parse(await readFile(output, "utf8"));
  const updatedReadme = await readFile(readme, "utf8");
  expect(latest).toMatchObject({ schemaVersion: 1, viral: { passed: 5, total: 5 }, react: { passed: 9, total: 10 } });
