...
function readmeBlock(snapshot) {
  ...
  return [
    "<!-- benchmark:start -->",
    `**${snapshot.viral.passed}/${snapshot.viral.total} viral cases passing. ${snapshot.react.uiVerified}/${snapshot.react.total} React cases UI-verified. ${snapshot.external ? `${snapshot.external.passed}/${snapshot.external.total} external third-party cases UI-verified. ` : ""}Original repo pollution rate: ${snapshot.react.originalRepoPollutionRate}.**`,
    "",
...
export async function updateBenchmarkSummary({ viralPath, reactPath, externalPath, outputPath, readmePath }) {
  const [viral, react, external] = await Promise.all([
